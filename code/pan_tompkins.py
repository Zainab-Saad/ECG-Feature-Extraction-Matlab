...
class Pan_Tompkins_QRS():
  def __init__(self, filename):
    ...
    self.record = wfdb.rdrecord(self.filename, sampfrom=180, sampto=4000,)    
    self.annotation = wfdb.rdann(self.filename, 'atr', sampfrom=180, sampto=4000,shift_samps=True)
  ...
  def derivative(self,signal):
    ...
    for index in range(len(signal)):
      ...
      result[index] = (result[index]*self.annotation.fs)/8
  ...
  def moving_window_integration(self,signal):
    ...
    result = signal.copy()
    win_size = round(0.150 * self.annotation.fs)
    sum = 0
